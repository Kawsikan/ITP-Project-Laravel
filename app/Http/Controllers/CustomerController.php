...
{   
    ...
    public function getCompletedCOrders(){
        $corder = Catering_order::all()->where('cstatus','completed');
        $count = Catering_order::all()->where('cstatus','incomplete')->count();
  
        return view('catering_orders.compcorder', ['corders' => $corder, 'count'=>$count]);
  
       }
       public function markAsCCompleted($id){
          $corder = Catering_order::findorFail($id);
          $corder->cstatus = "completed";
          $corder->save();
  
           return redirect('/show_corders');
       }
